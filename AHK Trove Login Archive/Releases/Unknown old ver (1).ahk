...
login_account(num) 
{
	if num = 1
		{
		ControlClick, x155 y253, Glyph Login
		Sleep, %normal_sleep%
		ControlClick, x155 y253, Glyph Login   ;to make sure any text left behind is deleted
		Sleep, %normal_sleep%
		ControlClick, x155 y253, Glyph Login
		Sleep, %normal_sleep%
		...
		WinWaitNotActive, Glyph Login,, 2 ; Wait for menu to go away
		if ErrorLevel
		{
	...
	if num = 2
		...
		WinWaitNotActive, Glyph Login,, 2
		if ErrorLevel
		{
			Sleep, %wait_for_login%
		}
