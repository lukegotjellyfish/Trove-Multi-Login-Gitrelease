...
Global normal_sleep            := 100    ;delays used throughout the script to make it work
Global wait_for_trove_to_open  := 4      ;delay in seconds for trove to open (runwait)
Global delay_time_in_ms        := 10     ;delay after each keypress
Global how_long_until_restart  := 10000  ;delay (1hour(3600000) for how long to wait before restarting
global Transparency            := 220    ;0~255
global FontSize                := 10     ;status GUI font size (top left)
...
;############################################################################################################
Gui, Add, Tab, x-6 y0 w250 h188, Launch Options|Info
Gui,+AlwaysOnTop
...
Gui, Add, Button, x22 y145 w200 h40 glaunchthemainscript, Launch
...
Gui, Show, x100 y225 h188 w240, Launcher
return
...
If RestartEveryDelay = 1
{
	loop
	{
		...
		if (window_auto_delete = 1) and OpenBackpack
			{
				open_backpacks_properly_baka(1)
...
Else if !(RestartEveryDelay)
	{
		...
		if (window_auto_delete = 1) and (OpenBackpack = 1)
		{
			open_backpacks_properly_baka(1)
			MsgBox,,, Press F4 to stop item deleter, 0.5
			run, invdelete.ahk %passover%
		}
		if (window_auto_delete = 1) and !(OpenBackpack)
		{
			open_backpacks_properly_baka(1)
...
login_account(num) 
{
	if num = 1
		...
		IfWinExist, Connexion à Glyph
			{
				global glyphloginname
				glyphloginname = Connexion à Glyph
			}
...
CreateGUI() 
{
	...
	Gui, Add, Text, x3 y3 w110 h20 +Border, Restart script:
	Gui, Add, Text, x120 y3 w21 h20 +Border vthing1status
	
	Gui, Add, Text, x3 y33 w110 h20 +Border, Open backpack:
	Gui, Add, Text, x120 y33 w21 h20 +Border vthing2status
	
	Gui, Add, Text, x3 y63 w110 h20 +Border, Launch fishing bot:
	Gui, Add, Text, x120 y63 w21 h20 +Border vthing3status
	
	Gui, Add, Text, x3 y93 w110 h20 +Border, Delete trophies/boots:
	Gui, Add, Text, x120 y93 w21 h20 +Border vthing4status

	WinSet, Transparent, %Transparency%
	Gui, Show, x0 y0 h176 w150
}
...
GuiControl,     ,vthing1status, %thing1status%
GuiControl,     ,vthing2status, %thing2status%
GuiControl,     ,vthing3status, %thing3status%
GuiControl,     ,vthing4status, %thing4status%
Gui, Show, x0 y0 h176 w150
}
